...
CREATE TABLE IF NOT EXISTS webhook_subscriptions (
    ...
    name VARCHAR(255) NOT NULL,
    url TEXT NOT NULL,
    secret TEXT NOT NULL, -- encrypted at rest (secretcrypt); widened from VARCHAR(255) by migration v65
...
-- Grant DML to the runtime app role for vault tables when present (mirrors migration v56).
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'openidx_app') THEN
...
-- PAM rotation engine (M1b): credential_rotation_policies + credential_rotations
-- ledger reconcile. Mirrors migration v57. Both tables under the v37 FORCE-RLS belt.
...
-- v37-style RLS belt for credential_rotation_policies (mirrors migration v57).
...
-- Grant DML to the runtime app role for rotation tables when present (mirrors migration v57).
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'openidx_app') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON
...
-- guacamole_session_requests and guacamole_sessions, both under the v37 FORCE-RLS belt.
-- ============================================================================
...
-- v37-style RLS belt for guacamole_session_requests (mirrors migration v59).
...
-- v37-style RLS belt for guacamole_sessions (mirrors migration v59).
...
-- Grant DML to the runtime app role for session tables (mirrors migration v59).
-- Plain GRANT (no DO $$ block) matching the migration runner's splitSQL constraint.
