...
#### Registering the custom URL scheme (do this after every `flutter create`)

Two of the four `openidx://` links arrive from **outside** the app and must be
routed by the OS: `openidx://oauth-callback` (the server's 302 after login) and
`openidx://enroll` (the QR-free enrolment link). The other two —
`openidx://qr-login` from the camera scanner and `openidx://approve` from a
notification tap — are already inside the app and need no registration.

Android routes both through the **committed**
`android/app/src/main/AndroidManifest.xml`. iOS cannot: `/ios/` is generated by
`flutter create` and Flutter's template carries no `CFBundleURLTypes`. So run
this from the repository root, **after** materializing the iOS runner and before
building:

```bash
flutter create --platforms=ios,android --project-name openidx_client .   # in client/
cd .. && bash scripts/ci-configure-ios-deeplinks.sh
```

It is idempotent, it reads the scheme out of the Android manifest rather than
repeating it, and it fails loudly if the plist is not there — because a silent
no-op is exactly how the scheme went unregistered for as long as it did. Every
CI job that materializes an iOS runner calls it, and
`scripts/check-ios-deeplink-config.sh` fails the build on the next one that
does not.
