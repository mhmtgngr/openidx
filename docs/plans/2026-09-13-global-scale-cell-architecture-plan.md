...
- [ ] **Kalan — ADMIN düzlemi sorgularının `Reader()`'a taşınması.** Depoda bunun yerleşik bir deseni var: her depo için mutasyon-testli bir muhafız (`WritesNeverUseReplica`, `ReadsUseReplica`, `SecurityCriticalReadUsesPrimary`, `SettingsRepositoryGetUsesPrimary`). `internal/admin` ~40 dosya ve her biri kendi oku-yazdıktan-sonra kararını gerektiriyor; **2.1b'nin dersi** bunun toplu düzenleme olarak yapılmaması gerektiği. Özellik büyüklüğünde partiler hâlinde, her biri muhafızıyla.
...
- [ ] **Kalan — dağıtım yarısı.** Roller var ve şifresiz (v53 gibi, şifre dağıtımda dışarıdan). Bir servisi bir düzleme bağlamak `DATABASE_URL`'ını o role çevirmek demek; **chart'a henüz eklenmedi.** İki gerçek soru var: (a) `identity-service` bugün hem ISSUE (auth uçları) hem ADMIN (CRUD) sunuyor — tasarım §4.1 onu ikiye ayırmayı öngörüyor ama ayrım yapılmadı, bu yüzden bugün **daha gevşek** olanı (ADMIN/10 s) almalı; daha sıkısı çalışan işlevi keserdi. (b) pgcat açıkken düzlem rolleri havuzun `users.N` girdileri olmak zorunda, yani backend bütçesi üçe bölünür — bu tasarım kararı verilmeden chart'a bir şey eklemek yanlış olur.
- **Kabul:** ADMIN'de 15 s'lik yapay sorgu → iptal; ISSUE etkilenmez. *(Ölçüldü ve CI'da koşuyor. Düzlemlerin gerçekten o rollerle bağlanması yukarıdaki kalan kalemde.)*
...
- [ ] **Kalan:** SCIM `/Users` (`internal/provisioning`) ve `internal/audit/reports.go` + `service.go:1157` hâlâ `OFFSET`. SCIM ayrıca `startIndex`/`totalResults` (RFC 7644) geri uyumluluğu gerektiriyor, ki audit'te olmayan bir kısıt.
...
- [x] identity-service `SERVICE_PROFILE=auth|admin`: auth profili yalnız login/MFA/passwordless rotalarını kaydeder. **(2026-09-13)**
  ...
  - **Kalan:** iki Deployment'ı render eden Helm yarısı ve kenar yönlendirmesi — rota yolları önek ile ayrılamıyor (`POST /users/forgot-password` ISSUE, `GET /users` ADMIN), dolayısıyla tek bir `pathType: Prefix` kuralı yetmez; manifestten üretilen kural listesi gerekiyor.
