...
| J4 | **Network access** — enroll agent/BrowZer → posture check → reach a dark service (end user) | ✅ | Real: Windows/Android agents, BrowZer clientless, posture checks, reconciler, `tools/darkprobe` proves dark services are reachable only by authorized identities, and a "going dark" runbook exists. Per-app scoping of dial policies arrives with the same rollout as J3. *Fixed on this branch:* **the posture endpoint that decides a device's network tier accepted reports from anyone.** `/agent/report` and `/agent/config` sit outside the JWT middleware because an agent authenticates with its own enrollment credentials — and neither handler read them, so an unauthenticated request could grant a device `device-trusted` (the Tier‑2 gate) or strip a compliant laptop of it. Both shipped agents already send the credential and the spec already documented the 401. Now proved end to end by `test/integration/network_access_test.go`, and the class it belongs to is closed by `internal/access/public_surface_test.go`: of the service's 325 routes, the fourteen that answer without the tenant‑JWT middleware are now *derived* (registered against a stub middleware that refuses everything — a route answering anything else is outside it by construction) and each must be declared with the shape of its refusal and a reason. |
...
3b. ◐ **Retire the registers** (the 61 + 34, in batches by domain: audit ·
   ...
   **Batch 19 shipped (migration v157, `needsScoping` 24 → 22; registers 41 →
   39): the key was the primary key, and a risk engine reading a table nobody
   writes.** Both tables are v62's, and v62's own note says what it left
   undone — *"Not under the v37 FORCE-RLS belt: the code does not org-scope
   these (no org_id in any query) … org_id/RLS is a separate hardening
   follow-up."* `breach_incidents` and `breach_alerts`, the other two of its
   four tables, were scoped in an earlier batch. These are the unkept half of a
   written-down promise.

   `admin_console_settings` is `key TEXT PRIMARY KEY`, and the handler writes
   exactly four keys — `general`, `security`, `authentication`, `branding`. So
   the installation had **four settings rows in total**, and every
   organization's administrators shared them. The read carried no predicate of
   any kind:
   ...
   and the write was an upsert `ON CONFLICT (key)`. This is the **sixth
   install-wide key** this programme has found, after v138's
   `ispm_rules.check_type` and `ai_agents.name`, v155's
   `federation_rules.email_domain` and `identity_providers.issuer_url`, and
   v156's `developer_settings.setting_key` — and the most severe of the six, for
   two reasons. The key is the *primary* key rather than a spare `UNIQUE`
   beside one. And of what the four rows hold: the `security` row carries the
   password policy (minimum length, character classes, forbidden words, maximum
   age, history depth), the MFA settings including **whether MFA is required**
   and which methods are allowed, the WebAuthn relying party and the session
   timeouts; `authentication` carries the allowed sign-up domains. One
   administrator lowering their minimum length to eight, or turning MFA-required
   off, did it for every tenant — and each tenant's settings page was showing
   them whatever the last administrator anywhere had saved.

   **Unlike v156's `developer_settings`, this one has a consumer.**
   `getPasswordPolicy` reads `WHERE key = 'security'` and
   `POST /api/v1/settings/validate-password` answers from it, so the shared row
   was not merely displayed: it was the policy a password was measured against.
   Re-scoped to a `(org_id, key)` primary key.

   **The second table had no writer at all.** `auth_contexts` is read at three
   sites in `continuous_auth.go` and written at none — not by a handler, not by
   the login path, not by a seed or a migration. The one `UPDATE` is the whole
   of "update the authentication score", so it matched no rows and reported
   success. The `SELECT` is the first statement of `CalculateSessionRisk`, so
   that function returned *"no rows in result set"* for every session that has
   ever existed and all three registered routes — `GET /continuous-auth/risk`,
   `POST /continuous-auth/check`, `POST /continuous-auth/update` — had only ever
   returned 500.

   The scan proves it never once ran against a row: it selects the `TEXT` column
   `location` into `&authCtx.Location`, and `AuthContext.Location` was
   `*GeoLocation`, so the destination was a `**GeoLocation` and the scan would
   have failed on type every time.

   So there was **nothing here to scope**. Adding `org_id` and a policy to a
   table with no rows and no writer moves a name off a register and changes
   nothing — the ceremony this programme exists to stop. What the engine needed
   was the data it was written to read, which already exists, already carries
   `org_id`, and is already behind the belt: `sessions` (belted since v37,
   written by `identity/session_repository.go` with `org_id`) for the user,
   address, agent, auth time and auth methods, and `session_risks` (v77,
   `org_id NOT NULL`) for the score `CalculateSessionRisk` already writes and
   reads back. `auth_contexts` was a shadow of both, and the shadow is what got
   read. v157 drops it; the down migration recreates it verbatim.

   Every *other* factor in the engine already resolved `orgctx` and scoped its
   own query — `known_devices`, `audit_events`, `risk_factors` and
   `session_risks` all carry `AND org_id = $N`. The tenant term was missing at
   exactly one place: the input.

   Three further defects in that file become reachable the moment the engine
   works, so they are fixed in the same commit. **The thresholds were zero on
   two of the three routes**: only the risk route built a full config; check
   built `&ContinuousAuthConfig{SessionMaxLifetime: 8h}` and update built an
   empty one, and `determineRiskLevel` tests `score >= CriticalRiskThreshold`
   first — so with a critical threshold of zero every session on those routes
   graded `critical`, which `determineActionRequired` turns into **`terminate`**.
   All three now share one `defaultContinuousAuthConfig()`. **An unknown event
   was scored as zero and reported as recorded**: the `switch` fell through, so
   a caller mis-spelling `failed_auth` was told their report had been stored; it
   is now a 400 that lists the accepted events. And **the caller's metadata was
   discarded** — the body marshalled the context's own map and ignored the
   argument — so it is now recorded with the event.

   One factor is reported as *unavailable* rather than guessed. `calculateDeviceRisk`
   needs a device fingerprint, and there is no honest way to get one for a
   session: `sessions` carries `device_name` and `device_type` but no
   fingerprint, and `login_history` — which does record `device_fingerprint` —
   has no `session_id` to join on. Asserting the 25 of an unknown device on
   every session would be a constant dressed as a measurement, the same shape as
   the flat 15 this file's IP factor used to return under the name of impossible
   travel. It reports "not measured", contributes nothing, and the response's
   `context.factors_measured` says so, so a caller acting on a score knows which
   factors built it. **Recording a fingerprint at login is an outstanding
   feature**, named here rather than guessed at.

   Proven on Postgres 16: nine cases across two tests in `internal/admin`, and
   the red-proof turned five assertions red against neutralised predicates — org
   B reading org A's settings back out of the shared row, a password that fails
   org B's 20-character policy being accepted because org A's 8-character policy
   was the one on file, `updated_by` coming back as org A's administrator on org
   B's read, and org B both scoring and pushing a `failed_auth` event onto org
   A's session. `TestRLSBeltTables` **54/54**; v157 applied, rolled back to 156
   — recreating `auth_contexts` and restoring the single-column primary key —
   and re-applied.

   One observation recorded rather than acted on: the repository carries a
   **second migration tree**, `migrations/*.sql` (105 files, most recent
   `202607030002`), which nothing applies. `openidx migrate` shells out to
   `cmd/migrate`, which uses `internal/migrations`; the root tree is read only
   by the scaffolding that creates new files, by `seed.go` for one seed file,
   ...
   **Batch 21 shipped (migration v159, `needsScoping` 20 → 16; registers 37 →
   33): v44's four tables — two get a tenant, two go.** v44 was a reconcile
   pass that lifted four tables out of loose SQL files so managed-Postgres
   installs would stop 500ing, and its own note records the choice it deferred:
   *"Not org-scoped (matches source + handlers), consistent with v42/v43."*

   A `kiosk_policies` row puts a managed device into single-app or multi-app
   lock task mode: which packages may run, which activity is pinned to the
   screen, the branding shown, and the hash of the PIN required to leave. The
   admin list carried the finding **in its own doc comment** —

   ```go
   // HandleListPolicies returns every kiosk policy (admin view, no filtering).
   ```

   — and get, update and delete addressed a policy by bare id. So one tenant's
   administrator could read another's lockdown estate, disable it, or delete it,
   with nothing on the owning console to say the control had stopped existing.

   **The assignment is the sharper half.** `HandleAssignPolicy` took the policy
   id from the URL and `target_id` from the body, checked neither, and wrote the
   row. `target_id` is a plain `VARCHAR(128)` naming an agent, and
   ...
   tenant's enrolled device, pinning it to one activity behind an exit PIN only
   they know. Both assignment handlers now verify the policy belongs to the
   caller.

   **What this does not close, stated plainly.** `enrolled_agents` has no
   `org_id` by a decision recorded in three separate comments in
   `internal/access`, so an agent id names a device without naming a tenant and
   there is no tenant term to put on `target_id`. v159 stops one tenant aiming
   *another tenant's* policy; it does not stop a tenant aiming *their own*
   policy at a device someone else enrolled, because "someone else's device" is
   not a thing this schema can express. **Whether the agent fleet is per-tenant
   is an open product decision**, and it belongs beside the other two the
   programme has declined to make unilaterally (`social_account_links` /
   `user_identity_links`, and the fleet tables themselves).

   **The belt and the agent path.** `/agent/config` resolves the effective
   policy for a device holding an agent token: no user, no organization on the
   context. Under the belt that read returns nothing, and the caller treats
   resolution failure as "no policy applies" and omits the block — so belting
   the table alone would have **silently unlocked every managed device on its
   next config poll**, the belt failing the operation closed rather than scoping
   it. `resolveEffectiveKioskPolicy` therefore runs under
   `orgctx.WithBypassRLS`, the same explicit opt-out the SAML and SSF
   pre-tenant paths use, and it is the one read of these tables that does.

   **Two tables whose only consumer was deleted.** v44 created
   `policy_recommendations` and `compliance_gaps` for, in its own words, the
   "AI policy suggestion / compliance-gap endpoints" in a handler file it names
   under `internal/admin`. That file is gone — commit `7f8d189e`,
   *"fix(admin): remove dead AI policy-recommendations route (WS-05)"*, removed
   it from this repository — which is why the citation is spelled out here
   rather than linked: the docs-drift guard checks every cited path against the
   tree and would, correctly, fail on a file that no longer exists. A search of the tree finds both
   table names in the migrations and in the orgscope register and nowhere
   else — no handler, no worker, no seed, no console call. They are dropped
   rather than scoped, for the reason v157 dropped `auth_contexts`: giving
   `org_id` and a policy to a table with no reader and no writer moves a name
   off a register and changes nothing. Nothing can have written a row, so
   nothing is lost, and the down migration recreates both verbatim.

   One pre-existing bug surfaced by the batch's own test and fixed with it:
   `HandleCreatePolicy` passed `string(req.AllowedPackages)` into
   `COALESCE($5,'[]'::jsonb)`. An omitted `json.RawMessage` stringifies to `""`
   rather than NULL, so the COALESCE never saw a NULL and Postgres was handed
   the empty string as `jsonb` — **every create that omitted
   `allowed_packages`, `lock_task_features` or `branding`, all three optional in
   the request type, failed with a bare 500.** The file already contains
   `nilIfEmpty`, written for exactly this and used by `HandleUpdatePolicy`;
   create never called it.

   Proven on Postgres 16: five cases in `internal/access`, and the red-proof
   turned **seven assertions red** against neutralised predicates — both tenants
   seeing both lockdown estates, org B reading, rewriting *and* deleting org A's
   policy (after which org A's own reads 404, the defect demonstrating itself),
   and org B aiming org A's policy at a device. The test also asserts the agent
   path still resolves the policy under the bypass, and that both orphan tables
   ...
   **Batch 23 shipped (migration v161, `needsScoping` 13 → 11; registers 30 →
   28): the actions were scoped and the record of them was not — for the third
   time — and Cancel was a lie.** A bulk operation is the console's "do this to
   these fifty accounts" control: enable, disable, DELETE, assign or remove a
   role, add to or remove from a group, force a password change.

   Every statement in `executeBulkOperation` carries a tenant term —
   `UPDATE users SET enabled = false ... WHERE id = $1 AND org_id = $2`,
   `DELETE FROM users WHERE id = $1 AND org_id = $2` — and the function opens
   with a comment recording an earlier fix in this same programme, that the
   organization has to travel on the *context* because the tables are belted and
   the pool reads `app.org_id` at checkout. Somebody thought carefully about the
   blast radius of the actions. **Nobody scoped the record.** The list read
   `FROM bulk_operations ORDER BY created_at DESC LIMIT 50` with no predicate at
   all, and `bulk_operation_items` stores `entity_name` — the **username** the
   item acted on — so opening another organization's run returned their
   directory, one row per account, with what was done to it and why it failed.
   The same cross-tenant read of personal data v154 found in the lifecycle run
   logs, and the third instance of this shape after v154 and v158.

   **Cancel was a lie, and it is fixed rather than recorded**, because it is a
   control and not a missing feature. `handleCancelBulkOperation` sets
   `status = 'cancelled'` on the row; `executeBulkOperation` never read that
   column. It walked every id it was given regardless, and then wrote
   ...
   left with no sign their cancel had been ignored. The loop now checks before
   each account and stops; the final write no longer clobbers a cancelled run.

   **A success that touched nobody**, also fixed here.
   `UPDATE users SET enabled = false WHERE id = $1 AND org_id = $2` against an
   id outside the caller's organization matches no row and returns **no error**,
   so `errMsg` stayed empty and the item was recorded `success`. A bulk disable
   over fifty ids belonging to somebody else reported fifty successes and
   changed nothing — the same shape as v154's `ExecuteLifecycleWorkflow`. An
   action that matched no row is now an error, with the reason recorded.

   And **the silent-drop pattern for the fifth time**: `entity_name` and
   `error_message` were scanned into plain strings with `continue` on failure,
   and `error_message` is NULL on every pending *and every successful* item — so
   the item list showed only the items that had already failed, and a run that
   succeeded looked empty. Precisely v154's `handleListLifecycleExecutions`
   defect, in a different file. COALESCEd, and the skip is logged.

   Proven on Postgres 16: six cases in `internal/admin`, and the red-proof
   turned **thirteen assertions red** against neutralised predicates and
   reverted behaviour — including the cancelled bulk disable disabling the
   account anyway, and the foreign id recorded as a success. Whole
   `internal/admin` suite green; `TestRLSBeltTables` **63/63**;
   `TestComposeMigrateSeedProducesRLSInstall` green.

   **Batch 24 shipped (migration v162, `needsScoping` 11 → 9; registers 28 →
   26): the switch was guarded on the way on and open on the way off.** A
   `service_features` row (v40) is a route's ZTNA switch — whether Ziti, BrowZer
   or Guacamole is on, the overlay service name and Guacamole connection id it
   provisioned, and the health verdict shown beside it. A `connection_tests` row
   (v54) is the stored result of the console's **Test connection** button.
   Neither table was given a tenant column.

   `EnableFeature` opens with `validateRouteTypeCompatibility`, which resolves
   the caller's organization and reads
   `SELECT ... FROM proxy_routes WHERE id = $1 AND org_id = $2` — so *enabling*
   a feature on another organization's route already failed with "route not
   found". **`DisableFeature` had no such lookup anywhere in its path.**
   ...
   all addressed the row by route id alone. One administrator could turn off
   another organization's ZTNA overlay for a route by naming its id.

   **And the teardown ran before the write.** `deprovisionFeature` for the Ziti
   feature reads `ziti_service_id` out of the row it was handed and calls
   `DeleteService` against the controller, so on an install where the reconciler
   does not own teardown the foreign organization's overlay **service** was
   deleted, not merely marked off. What did *not* happen is the part that would
   have shown on their console: `syncRouteFlags` — the one statement in
   `DisableFeature` that *was* org-scoped — matched no row, so
   `proxy_routes.ziti_enabled` stayed `true`. The owning tenant was left with a
   route the console reports as Ziti-protected whose Ziti service no longer
   exists. A control that displays without enforcing, arrived at from the other
   side.

   **The test history is internal topology.** `handleGetConnectionTestHistory`
   read `FROM connection_tests WHERE route_id = $1` with no tenant term and no
   check that the route belonged to the caller, and the `details` JSONB it
   returns is the whole per-test result map: `url` is the route's upstream
   `to_url`, `address` is the `host:port` a TCP probe dialled, `service_name`
   and `service_id` are the overlay service, and `error_message` is the raw dial
   error, which names the host it could not reach. A route id was enough to read
   another organization's internal hostnames and ports out of the product — the
   class this repository runs a dedicated CI guard for on its *public* surfaces,
   sitting unscoped behind an admin one. The red-proof printed org A's
   `payroll.internal.orga:8080` into org B's response, twice.
   `handleTestConnection` went through `getRouteByID`, which applies the tenant
   filter *when* the context carries an organization and silently drops it when
   it does not — right for the data plane, the wrong direction for a request —
   so it now requires one.

   **A measurement that was taken and thrown away**, fixed here rather than
   recorded because the probe already exists. `UpdateFeatureHealth` is the only
   writer of `service_features.health_status` in the tree and **had no caller**:
   not a worker, not a handler, not the health engine. So the coloured dot beside
   each feature on the Zero Trust page (`zero-trust.tsx:368`) and the badge on
   the route's feature panel (`ServiceFeaturePanel.tsx:190`) have read `unknown`
   for every feature on every route since v40 — honest, but unable to be
   anything else. The connection test in this same batch dials the upstream,
   resolves the Ziti service and validates the Guacamole connection, and stored
   its answer in `connection_tests` and nowhere else. The test now also records
   each feature's outcome, so the dot means "the last connectivity test said
   this" and stays `unknown` until one has run. A probe that did not run leaves
   the previous verdict alone: "not measured this time" is not a verdict.

   Proven on Postgres 16: five cases in `internal/access`, and the red-proof
   turned **twelve assertions red** against neutralised predicates and a
   reverted call — including org B disabling org A's overlay, the internal
   hostname printed into org B's body, and both handlers answering a caller with
   ...
   **Batch 26 shipped (migration v164, `needsBelt` 17 → 14; registers 22 →
   19): the first of the belt register — the belt v95 said it had, and a
   predicate that carried its own bypass.** This is the register's other half:
   tables that *do* carry `org_id` and never received FORCE ROW LEVEL SECURITY,
   so the database does not enforce the boundary and one forgotten predicate
   crosses tenants silently. The register's own note says several of their
   migrations *"say org-scoped for RLS while the belt was never applied"* —
   v95's registry description is the clearest instance, because it says exactly
   that, in full: **"Org-scoped for RLS."** Each of its three outbound-SCIM
   tables got an `org_id UUID` column. None got the belt, a policy, a
   `NOT NULL`, or a foreign key.

   **What is live: a NULL tenant.** `org_id UUID` with no `NOT NULL`, written
   through `nullIfEmpty(orgID)`, means a row can be stored with no tenant at
   all — and a NULL-org row is invisible to every org-scoped read once the belt
   lands. It does not leak, it **disappears**: a provisioning connection that is
   still pushing users to Slack stops appearing on the console, and a queued
   operation the worker still drains becomes one no operator can see. That is
   the hazard the register records against `edr_device_mappings`, which is why
   the column is made `NOT NULL` with a real foreign key here rather than merely
   belted.

   **What is latent, and worth fixing anyway: the predicate carried its own
   bypass.** Every "org-scoped" query in the store reads
   `WHERE id = $1 AND (org_id::text = $2 OR $2 = <empty>)` — on get, list,
   update and delete, and again on the two `INSERT..SELECT` fan-outs, where it
   decides which **users** are enqueued. An empty organization means *every*
   organization: `ListTargetApps` returns every tenant's SCIM connections,
   `DeleteTargetApp` removes any of them together with their records and queued
   operations by cascade, and `EnqueueFullSync` pushes the whole installation's
   directory — every username, email and name — into one tenant's downstream
   SaaS.

   It is **not reachable through the mounted routes today**, and the guide says
   so rather than overstating it: the provisioning service runs `TenantResolver`
   in front of every route, no provisioning path is on the resolver's skip list,
   and the resolver either attaches an organization or aborts, so the helper
   that produced the empty string cannot in fact do so. The bypass is one
   middleware change, one skip-path entry, or one in-process caller away from
   being load-bearing, and the store methods are exported precisely so
   identity's SCIM write paths can call them.

   **The bypass was for the tests.** Every production caller of the fan-out
   passes a resolved `org.ID` — `service.go` does so at all three inbound SCIM
   write paths. The only callers in the tree that passed the empty string were
   the package's own tests, and one of them (the full-sync handler case) was
   passing *because* of the wildcard: it seeded users with a NULL organization
   against a router that attached none. The escape hatch existed to make a test
   convenient and left the production code carrying a wildcard.

   It also contradicts the belt outright: under FORCE ROW LEVEL SECURITY a query
   with no `app.org_id` returns nothing, so the escape hatch cannot mean "every
   organization" any more — it means "no rows", silently. Predicate and belt now
   agree, and the handlers **refuse** a request without an organization.

   **The worker keeps its bypass, and already had one.** The register asked to
   *"confirm the outbound worker's scope before belting"* this queue. It passes:
   `StartOutboundWorker` runs its loop under `orgctx.WithBypassRLS`, the drain
   claims by `state` and `next_attempt_at` and never by tenant, and each claimed
   row carries its own organization onward. Its five by-id statements now carry
   `//orgscope:ignore` with the reason — the self-closing ratchet caught all
   five the moment the tables left the register, which is the coupling the
   register was built for.

   Proven on Postgres 16: eight cases in `internal/provisioning`, and the
   red-proof turned **ten assertions red across five subtests** — including a
   caller with **no organization at all** deleting org A's SCIM connection
   (`{"status":"deleted"}`), whose cascade then broke the fixtures the later
   subtests needed, the defect destroying the evidence as it demonstrated
   itself. Whole `internal/provisioning` suite green; `TestRLSBeltTables`
   **70/70**; v164 applied over a seeded NULL-org row that followed its target
   to the right tenant rather than the oldest-org fallback, rolled back to 163
   and re-applied. The package's DB harness gained the same
   `OPENIDX_TEST_DATABASE_URL` escape hatch `internal/admin` grew in v138, so
   this batch could be proven locally against real Postgres rather than only in
   CI.

   **Batch 27 shipped (migration v165, `needsBelt` 14 → 12; registers 19 →
   17): a device match that crossed tenants into an enforcement decision.**
   ...
   `needsBelt` register has carried a **specific warning against
   `edr_device_mappings`** since it was written, cited by both v150 and v164:
   the ingest writes a NULL `org_id` when the source has none, so a belt would
   *hide* rows rather than scope them.

   **The finding is not a read.** `resolveIdentityForDevice` turns an EDR device
   report into a local Ziti identity, and all three of its match strategies
   ...
   — so a device reported by **one** organization's EDR connection was matched
   against **every** organization's identities, with `LIMIT 1` and no `ORDER BY`
   leaving the choice to the query planner. What that identity is used for is
   the point: it goes straight to `RecordPostureResult`, and a *failing* posture
   result is what the proxy and continuous verification read to revoke the
   session and sever the overlay circuit.

   So the consequence is a **revocation**, not a disclosure. One tenant's EDR
   calling a laptop non-compliant could fail the posture check on another
   tenant's user and cut their access, on nothing more than a shared email
   address, hostname or device serial — and one person employed by two
   organizations on one installation is the ordinary case, not a contrived one.
   The direction of the error is deny, so it costs availability rather than
   granting anything, which is why it is stated as such rather than as an
   escalation. All three queries now carry the source's organization, matched on
   `ziti_identities.org_id`; that also closes the two strategies that reach
   through `enrolled_agents`, which has no tenant of its own — an agent enrolled
   by a user in another organization cannot join to an identity in this one, so
   the fleet decision this programme has twice deferred does not block the fix.

   **The same wildcard as v164**, and worse in one place: get, list and delete
   carried the OR-empty-string escape hatch, and `connectorForSource` passed it
   **explicitly** — `GetEDRSource(ctx, "", id)` — while itself reading the
   source's encrypted API credentials by bare id and decrypting them into a live
   connector. As in v164 this is not reachable through the mounted routes, and
   as in v164 it contradicts the belt. The wildcard is gone, the handlers refuse
   a request with no organization, and the one caller that legitimately spans
   tenants is now an explicitly-named function rather than a wildcard call.

   The ingestion worker keeps the RLS bypass it already had:
   `StartEDRIngestionWorker` runs the poll loop under `orgctx.WithBypassRLS`,
   and the due-sources sweep is install-wide by design — a source the sweep
   cannot see never polls, and a device that stops reporting must still age out
   to failing.

   Proven on Postgres 16: six cases in `internal/access`. The first red-proof
   was **too weak and was rewritten**: with the tenant term removed, the shared
   email address still resolved org A's own identity, because `LIMIT 1` without
   an `ORDER BY` returned whichever row the planner reached first. The test now
   also reports a device whose address exists **only in org B**, which cannot
   pass unscoped — and the red-proof then names it outright: *"org A's EDR
   source resolved …8fdbc1c9 for an address that exists only in org B."* Three
   assertions red. Whole `internal/access` suite green; `TestRLSBeltTables`
   **72/72**; v165 applied over a seeded NULL-org mapping that followed its
   source to the right tenant, rolled back to 164 and re-applied.

   **Batch 28 shipped (migration v166, `needsBelt` 12 → 10; registers 17 →
   15): a revocation that reported done when it had done nothing.** v100 and
   v101 are the hand-off between two services — governance decides, and the
   access-service, which owns the Ziti manager, applies. A grant row adds the
   time-bound role attribute that opens the dial; a revocation row removes an
   attribute and severs the user's live circuits.

   **The register's check passes.** Its note said to confirm the reconciler's
   scope before belting these, and both workers already run their drain loop
   under `orgctx.WithBypassRLS`: one worker serves the whole installation,
   claims by `state='pending'` and never by tenant, and each claimed row carries
   its own organization onward. Neither queue has a request-path reader at all,
   so there is no cross-tenant read surface — here the belt is the database
   enforcing what the application already does.

   **The finding is the organising defect, pointed at the direction that matters
   most.** `drainNetworkRevocations` called `removeUserZitiAttribute`, which
   returned nothing at all, and `severUserZitiCircuits`, which logged a
   termination failure and carried on — and then wrote
   `SET state='done'` **unconditionally**. So an access review revoked a user,
   governance recorded the revocation, and the queue recorded the circuit
   severance as done **even when the Ziti call failed or the controller was
   unreachable**. The user's live circuit stayed open and every record said it
   had been cut.

   **Its own sibling got this right.** `drainNetworkGrants`, in the file next to
   it, checks the error and writes `state='failed', last_error=…`. One direction
   of the same pair recorded its failures and the other did not — v162's shape
   again (guarded on the way on, open on the way off), here as *recorded on the
   way in, silent on the way out*.

   **And `failed` had no reader, and no retry.** Nothing in the tree reads
   `state` or `last_error` from either queue: no handler, no console page, no
   alert. So even the grant worker's honest failure was a row nobody would ever
   see — an administrator approves a network access request, the console says
   granted, the attribute was never added because one Ziti call failed, and
   there is no surface that says so. Both queues also carry an `attempts` column
   **neither worker has ever incremented**: the column was authored for a retry
   that was never written, so a single transient controller error lost the grant
   or the revocation permanently.

   Fixed rather than recorded, because a revoke that reports success without
   revoking is a control and not a missing feature: both Ziti helpers now return
   their errors, a failed item is retried against the `attempts` column that
   already existed, and an item that exhausts its attempts is dead-lettered
   **and written to `audit_events` under the queue row's own organization** —
   which is what finally gives `failed` a reader, on a surface that is already
   org-scoped and already belted.

   **A pre-existing test caught a silent drop in the new code.** Adding `org_id`
   to the claim's `RETURNING` made the scan fail on a legacy NULL-tenant row,
   and the claim loop's `if err == nil` then dropped it — an intent claimed,
   marked `processing`, and forgotten, with the circuit still open. Three
   existing worker tests went red and named it. Both claims now COALESCE the
   tenant and log a skipped row instead of swallowing it, and the three tests
   pass unchanged.

   Proven on Postgres 16: five cases in `internal/access`, and the red-proof —
   attempts never incremented, dead-letter never audited, the attribute failure
   swallowed — turned **six assertions red across four subtests**. Whole
   `internal/access` suite green; `TestRLSBeltTables` **74/74**; v166 applied
   over two seeded tenantless rows and left none, rolled back to 165 and
   re-applied.

   **Batch 29 shipped (migration v167, `needsBelt` 10 → 9; registers 15 → 14):
   ENABLE is not the belt.** `group_application_assignments` is the register's
   highest-blast-radius entry — the table the enforcement path and the proxy's
   cross-org cache warmer both read — and it belts on its own, as the register
   said it would.

   v136 created it and gave it, in its own words, *"Org isolation: rows are
   visible only within their org unless a trusted caller sets app.bypass_rls
   (mirrors user_application_assignments, migration v37)."* It wrote
   `org_id UUID NOT NULL`, `ENABLE ROW LEVEL SECURITY`, and the same
   USING/WITH CHECK policy every other belted table has. **It did not write
   FORCE.**

   `ENABLE` applies a policy to every role *except the table's owner*; `FORCE`
   is what applies it to the owner too. So this table's isolation held only
   while the role that ran the migration and the role the application connects
   as were different roles. On the reference deployment they are —
   `deployments/docker/bootstrap.sql` creates `openidx_app` as
   `NOSUPERUSER NOBYPASSRLS`, and the Helm values note that migrations connect
   as the database owner — **so this was not a live hole there**, and the guide
   says so rather than overstating it. On any install where those roles coincide
   (a single-role Postgres, a managed service whose application user owns the
   schema, a developer's local database) the policy silently stops applying to
   the application, on this table alone.

   **And the proxy's own comment asserted otherwise.**
   `proxy_assignment_cache.go` explains why the data plane re-applies
   `orgctx.WithBypassRLS`, and its stated reason is that `applications`,
   `user_application_assignments`, `group_memberships` and
   `group_application_assignments` *"are all FORCE ROW LEVEL SECURITY."* Three
   of those four are. Read off a migrated database:

   | table | `relrowsecurity` | `relforcerowsecurity` |
   |---|---|---|
   | `applications` | t | t |
   | `user_application_assignments` | t | t |
   | `group_memberships` | t | t |
   | `group_application_assignments` | t | **f** |

   The group half of the app-assignment pair is the one that is not — the
   fourth time this programme has found a pair guarded on one side and not the
   other, after v154, v158, v161 and v162. The comment is corrected in the same
   commit as the migration, because a comment certifying a property nothing
   checks is how the property goes missing.

   The queries were already clean, which is why this table sat on `needsBelt`
   rather than needing a scoping batch: `appaccess`'s `assignedPredicate`
   carries `gaa.org_id = $2`, `PrincipalsFor` carries `AND org_id = $2`, and the
   portal's three handlers each carry their own term. This migration is the one
   statement that was missing.

   **A regression guard lands with it**, and it is the more durable half. The
   existing belt suite could not have caught this: its `requireForceRLS` helper
   **skips** a table that is not forced rather than failing it — a guard that
   steps aside for exactly the condition it exists to detect. The new
   `TestEveryPoliciedTableIsForced` asks the schema directly: every table with
   an RLS policy must also carry FORCE. Run against the schema as it stood
   *before* v167, it names `group_application_assignments` **and nothing else**
   — which is both the red proof for this batch and a confirmation that every
   other policied table in the installation was already forced.

   **Batch 30 shipped (migration v168, `needsBelt` 9 → 7; registers 14 → 12):
   one tenant could widen another tenant's allowlist.** `mcp_servers` and
   `mcp_tool_policies` are the MCP gateway's two tables: a server row is an
   upstream an AI agent may be proxied to, and a policy row says which principal
   — `client:<id>` or `role:<name>` — may call which tool on it. The policy table
   is an **allowlist**, so a row added to it grants; there is nothing else
   deciding the question.

   `AddMCPToolPolicy` took a server id from the URL and wrote a policy row
   against it **without ever checking the server belonged to the caller's
   organization**. The org column existed on both tables — nullable, no foreign
   key, no belt — and the write simply carried whatever the caller's context
   held. So an administrator of org B could name org A's server id and add
   `role:analyst → *` to it. **Role names are not globally unique**: org A has
   its own `analyst` role, and `toolAllowed` matched on the role *name* the
   caller's token carried. The rule org B planted was therefore evaluated
   against org A's analysts, on org A's upstream, and org A's own MCP policy
   page listed it as though an administrator there had written it.

   Everything around it was scoped the way the rest of the surface is —
   `getMCPServerByID`, `ListMCPServers`, `DeleteMCPServer` all carried a tenant
   term — which is precisely why this one was easy to miss: the *server* is
   protected and the *rule attached to it* was not. That is the same shape as
   v162's feature switch and v166's two queues: **a pair guarded on one side and
   not the other**, now for the sixth time.

   Four more OR-empty-string escape hatches came out in the same pass (an empty
   organization meaning every organization, on the server lookups the gateway
   and the approval path both use), `toolAllowed` and `toolRequiresApproval`
   gained their tenant term, `CreateMCPServer` and `AddMCPToolPolicy` now refuse
   a tenantless caller outright rather than writing a row no scoped read will
   ever return, and the four HTTP handlers answer **403** where they previously
   ran with no organization at all. The pending-approvals list gained
   `AND a.org_id = $1 AND (m.id IS NULL OR m.org_id = $1)` in its `WHERE` — not
   in the `LEFT JOIN`'s `ON`, which is the v155 lesson: a tenant condition in an
   outer join's `ON` filters nothing.

   As with the previous three batches the wildcards were latent rather than
   live — every mounted route resolves a tenant through `TenantResolver` before
   the handler runs — and the guide says so rather than overstating it. The
   allowlist widening was **not** latent: it needed only two administrators and
   a server id, both of which the console shows.

   The red proof ran six assertions across four subtests, and the last one is
   the defect demonstrating itself: with the fix reverted, the test's attempt to
   plant a foreign rule failed with a **unique-key collision (23505)** against
   the rule org B's `AddMCPToolPolicy` had *already succeeded* in writing onto
   org A's server.

   Two of the three MCP test files were themselves passing *because* of the
   defect — they ran as the empty organization and were matched by the
   wildcards. They now run as a real tenant.

   **Batch 31 shipped (migration v169, `needsBelt` 7 → 5; registers 12 → 10):
   a reason with a shelf life, and the guard a comment assumed.**
   `published_apps` and `discovered_paths` are the App Publish tables — register
   an internal application, discover its paths, publish them as proxy routes.
   v73 gave them an `org_id`, a backfill, a `NOT NULL` and a foreign key, fixed
   a real cross-tenant IDOR in their handlers, and then wrote down, in its own
   registry entry, why it stopped short of the belt:
   ...
   **Half of that reason had already expired and the other half was one line of
   plumbing.** The doctor does sweep every org — under `orgctx.WithBypassRLS`,
   which is *the belt's own documented opt-out*. A belted table is exactly as
   visible to it as an unbelted one; that half of the reason describes a path
   the belt was designed to permit. The goroutine genuinely ran on
   `context.Background()`, so the pool set no `app.org_id` and every statement
   would have matched nothing once a belt landed — but its SQL was never the
   problem: all six of its statements already carry `AND org_id = $N`, because
   the handler threads the org id in explicitly. What was missing was the org on
   the **context**, which is `orgctx.With(context.Background(), org)`.

   This is the second time in this programme that a written-down reason for
   skipping a control turned out to have expired — after v71's on the
   temporary-access tables, which had stopped being true three batches before
   anyone looked. A reason is a claim with a shelf life, and nothing was
   re-checking these.

   **And while reading the doctor: its two routes carried no role guard.**
   ...
   forty lines above that introduces `adminOnly` says exactly why: *"a plain
   authenticated tenant user must NOT reach them."* The doctor's own handler
   comment says the same thing in passing — *"an org-scoped **admin** request
   must see all rows"* — while the route it guards required no admin at all.
   **The guard the comment assumed is the guard nobody applied.**

   Because both handlers run under `WithBypassRLS` by design, RLS could never
   have closed this — the role gate was the only control there was. For any
   authenticated user of any tenant it meant:

   - `GET /health/relations` — an install-wide report naming every tenant's
     published app names, public hosts, OAuth `client_id`s and Ziti service
     names.
   - `GET /health/relations?heal=safe` — every Safe fix applied across every
     tenant, in one request.
   - `POST /health/fix/:checkId` — one named fix, including the two the code
     itself labels **RISKY**: tearing a service off the Ziti controller, and
     consolidating an app, which rewrites another tenant's proxy routes and
     repoints their `discovered_paths`.

   `FixOne` re-detects before it fixes, so a subject has to be a live finding;
   that bounds the reachable set, it does not scope it to the caller's tenant.
   Both routes now carry `adminOnly`.

   **The regression guard walks the real route table.** Rather than
   re-registering the two routes by hand — which would test a copy —
   `TestHealthDoctorRoutes_requireAdminRole` calls `RegisterRoutes` on a stub
   service, exactly as `test/openapi`'s spec coverage does, and issues real
   requests: a non-admin must be stopped at 403, an admin must get *past* the
   gate (503 from the handler's own nil-engine guard). With the gate reverted,
   six assertions go red and the body says why — `{"error":"health engine not
   initialized"}` on a plain user's request, which is the handler answering
   someone it should never have met.

   `discoveryContext` is now a named function with the reason on it, pinned by a
   test that fails with the exact operator symptom if the org is ever dropped
   again: *the app sits at "discovering" forever with nothing in the log to say
   why.* The reclassification handler — which decides whether a path is
   published, behind what auth, under what device-trust requirement — gained the
   cross-tenant case it did not have; with its predicate neutralised it returns
   **200 `classification updated`** on another tenant's `/admin` path.

   **Batch 32 shipped (migration v170, `needsBelt` 5 → 3; registers 10 → 8): a
   column nothing wrote, and the belt that would have broken the feature.**

   The register's note against `oauth_device_codes` said to *confirm the
   pre-tenant path before belting*, and that check passes for both tables in
   this batch. A device code is redeemed by the SHA-256 of the code the device
   holds, and a device polling `/oauth/token` is about as close to tenantless as
   this product gets — but it is not tenantless. `tenantSkipPaths` carries five
   entries (`/health`, `/metrics`, `/ready`, `/live`, and the login-branding
   bootstrap) and **no OAuth path is among them**, so every one of these
   requests is resolved by `TenantResolver` or refused with 400 before a handler
   runs. `loadDeviceCodeByHash`, `resolveUserCode` and `decideDeviceCode` already
   carry `AND org_id = $2`. Here the belt is the database enforcing what the
   application already does.

   `oauth_registration_tokens` was the other half, and it is the interesting
   one. v97 gave it an `org_id UUID` — nullable, no foreign key, no belt — and
   then **nothing in the tree ever wrote it**. The one INSERT names
   `(client_id, token_hash, created_at)`. Every row on every installation
   carries a NULL tenant.

   The register said to belt this table. **Belting it as it stood would have
   broken RFC 7592 client management outright.** Under FORCE ROW LEVEL SECURITY
   a NULL-org row matches no scoped read, so `registrationTokenValid` would have
   ...
   perfectly correct. **The column had to be filled before it could be
   enforced**, and that ordering is the whole content of the migration.

   That is the exact inverse of batch 31. There, a recorded reason for
   **skipping** a control had quietly expired. Here, a recorded instruction to
   **apply** one would have caused an outage. A register entry is a note, not a
   verdict; both of them wanted reading before they were acted on, and the
   register is a to-do list precisely so that reading happens once per table
   rather than never.

   **Orphans are deleted, not attributed** — a first for this programme's
   backfills. A registration token belongs to exactly one client
   (`oauth_clients.client_id` is UNIQUE across the installation and its `org_id`
   has been NOT NULL with a foreign key since v25), so the join is exact. A row
   whose client no longer exists cannot be validated by anything, and
   attributing it to the oldest organization would file another tenant's dead
   credential under theirs. The alternative to deleting it is inventing an owner
   for a credential.

   **Not a live cross-tenant hole**, and the guide says so rather than
   overstating it. The three unscoped DCR queries key on `client_id`, unique
   across the installation, so they were never ambiguous; and both management
   handlers read the client through the belted, org-scoped `oauth_clients`
   immediately after the token check, so a caller in another tenant already got
   404 there. The four device-code updates are the same shape — each takes a row
   id off a record already fetched *with* the tenant. What the tenant term adds
   is that these queries stop depending on the uniqueness of a column in a
   different table, and keep holding under an explicit RLS bypass.

   **Two red proofs.** Reverting the *writer* to v97's shape fails with
   `null value in column "org_id" ... violates not-null constraint` — the
   migration's own ordering demonstrating itself, since the column the belt
   depends on is now the one the INSERT must fill. Reverting only the *read*
   predicate lets another tenant validate the token: *"another tenant validated
   this registration access token."* And the device-code claim, unscoped, is
   redeemed by a tenant that does not own it — a claim being what mints the
   token.

   `internal/oauth`'s harness was testcontainers-only, so this batch could not
   have been proven on a machine without Docker. It gained the
   `OPENIDX_TEST_DATABASE_URL` path `internal/admin` grew in v138 and
   `internal/provisioning` in v164.

   **Batch 33 shipped (migration v171, `needsBelt` 3 → 0 — THE REGISTER IS
   EMPTY): a request that was never filed.**
   ...
   — *"files a pending device-trust request for an untrusted device that
   attempted access, so an admin can approve it."*

   v72 gave `device_trust_requests` an `org_id` and made it `NOT NULL`, with no
   default. The `INSERT` in `internal/access/device_trust.go` was never updated.
   So since v72 that statement has failed on **every** call with
   ...
   every time, on every install, for as long as the column has existed. A user
   locked out by device trust waited for an approval that was never in anybody's
   queue.

   The identity-side writer next door in
   `internal/identity/device_trust_approval.go` always wrote the tenant, and
   every read there carries `AND org_id = $N`. This is the pair's other half —
   the **seventh** time this programme has found a pair guarded on one side and
   not the other, and the **second** (after v163's entitlement Save) where the
   unguarded half turned out never to have worked at all. The difference is
   which way it fails: v163's returned a 500 the administrator could see; this
   one returned nothing, because nobody was waiting on it.

   **And a pre-existing test proved it worked.** `TestEnsureDeviceTrustRequest`
   builds its own fixture, and that fixture created `device_trust_requests`
   *without* `org_id` — a table shape the product has not had since v72. The
   test passed against a schema the constraint did not exist in. It now carries
   `org_id UUID NOT NULL` and a `users` row to resolve the tenant from, and with
   the fix reverted it fails with the operator's sentence rather than a
   constraint name: *"the proxy filed 0 device-trust requests, want 1."*

   **The background writer, again.** `report_exports` is scoped on create, list
   and download and unscoped in exactly one place: `updateExportStatus`, called
   from `generateReportAsync`, which ran on `context.Background()`. That is
   batch 31's discovery goroutine two batches later, and the same fix. Without
   it the belt would leave an export at `generating` for ever, with the file
   written to disk and the download endpoint answering *"export is not
   completed"* until somebody read the log.

   **The one genuine pre-tenant path in the whole register.**
   `enrollment_sessions` is read by the public `/agent/enroll` endpoint when an
   agent redeems its enrolment token: no JWT, no subdomain the resolver can
   trust to be the enroling tenant's, and on an install with the default-org
   fallback on it would be handed the **primary** org — the wrong one for a
   device enroling into any other tenant. Its in-tree comment said, correctly
   for its time, that *"enrollment_sessions is a global table like
   agent_enrollment_tokens, so no RLS bypass is needed for this
   high-entropy-keyed read"*. Belting the table makes that false, so the two
   public-path functions opt out explicitly with `orgctx.WithBypassRLS` and the
   comment is corrected in the same commit — the third standing claim this
   programme has had to retire alongside the change that invalidated it, after
   v167's proxy comment and v169's migration note.

   ### The belt register is empty

   `needsBelt` began this programme at **34** tables whose own migrations had
   drifted out of the v37/v121 belt — including several whose registry
   descriptions said "org-scoped for RLS" while the belt was never applied. The
   pin in `tools/orgscope/ddl_test.go` is now **0**, so the register cannot grow
   back: an `org_id`-carrying table without `FORCE ROW LEVEL SECURITY` fails the
   build outright. That is what the register existed to reach, and the ratchet
...
**The same invariant, one layer down: a setting an operator can set must be
read.** A configuration field is a control too — it is just displayed in a
config file or an environment variable rather than on a page — and it fails the
same way: `ENABLE_MFA=false` got you MFA, `PUSH_MFA_ENABLED=false` got you push
MFA, `SMS_OTP_LENGTH=8` got you six-digit codes, and the gateway's
`EnableRateLimit` let an operator set 100 requests a minute on an
internet-facing service that answered unbounded. Nothing else here can see it:
the field parses, the viper default makes it non-zero, the docs describe it, and
nothing fails at runtime, because the product does what it did before.

`tools/deadconfig` holds the settable set (every `mapstructure`-tagged field)
against the read set (`go/types`, tests excluded) and subtracts, as a merge-
blocking job. A finding's destination is `internal/common/config/retired.go`,
not a waiver: the field, its default and its binding go, and the name stays
...
|---|---|---|
| 1 · journeys verified | ☐ | J1 ✅ the `smoke` and `first-run` jobs (P6.2); J2 ✅ `test/integration/{auth_flows,mfa_flow,passwordless}_test.go`; J3 ✅ `test/integration/enforced_posture_test.go` (P6.1); J6 ✅ `test/integration/governance_loop_test.go` — the held browser spec stays held, and deliberately: it is 738 lines driven entirely by mocked responses, so promoting it would prove the console renders fixtures, not that a reviewer's decision does anything. The integration case drives the decision through the API against the running services and asserts the three effects separately — the item is recorded revoked, the role is actually gone, and the reviewed user's LIVE token stops working. The third crosses a process boundary (governance writes the marker, oauth-service reads it), which is the half that failed before and the reason `internal/revocation` exists; the integration job now boots governance-service so the two are really separate processes. Red-proofed by restoring the old divergent key: the first three assertions stay green and the fourth goes red, which is exactly the shape of the original defect; J7 ✅ `test/integration/leaver_test.go` — and writing it found the journey's third half broken: every sever path (identity's and provisioning's `deprovisionUser`, the access-service kill switch) published `revoked_session:<id>` markers, which the REFRESH grant honours, and none wrote the per-user revocation cutoff that `/oauth/userinfo` and `/oauth/introspect` actually read — so a disabled leaver's outstanding access token kept answering for the rest of its hour, and introspected `active: true`, while the console showed the account disabled. An access-review revocation, the least urgent control in the product, already cut those tokens; the kill switch did not. All three sever paths now call `revocation.RevokeUserTokens`; red-proofed by removing it, which turns the test's third and fourth assertions red while the login and refresh halves stay green — which is how it stayed broken; J5 ◐ `test/integration/privileged_access_test.go` — the credential half, driven against a running access-service with audit-service beside it (the integration job now boots seven). `jit_checkout_test.go` had covered the mechanics and said in its own header why it stopped there: "governance HTTP handlers are not driven (gin+JWT wiring is heavy)". The mechanics are not the control — the org predicate, the `allow_reveal` flag, the non-admin ACL and the audit row all live in the handler. Four assertions: an ungranted user refused, the granted user served, `allow_reveal=false` refusing **even an administrator**, and the reveal in the audit trail naming who and what. The last one found two defects, and the first is the worst thing this programme has turned up: `audit_events.id` is `uuid NOT NULL DEFAULT gen_random_uuid()` and `LogEvent`'s INSERT named the column, so an event arriving without an id put `""` into a uuid column and Postgres refused it — and `internal/access.logAuditEvent`, the endpoint's only caller in the product, has never sent one. Every audit event access-service emitted (every credential reveal, every entry created or deleted, every grant, every proxy allow and deny) was refused with a 500 and dropped, on every install, visible only as a warning line in the emitting service's log — and that line exists only because of an earlier fix on this branch. Proven in the endpoint's own responses before and after. The second: the event carried no `actor_id`, so the trail did not say who. J5's brokered-session half (guacd, a target host, a recording) stays an operator drill; J8 ◐ `test/integration/audit_chain_test.go` — the audit step, against the running trail: the product claims a tamper-evident log, so the test does what the control exists to catch and edits a sealed row directly, with the privileges an operator or an intruder with database access would have. The endpoint reports `intact:false`, names the event and prints the stored and computed hashes; restoring the row exactly makes it whole again, which is what stops `intact:false` being an answer the chain gives to everything. J4 ◐ `test/integration/network_access_test.go` — enrol and posture, the two steps that decide the third. `applyPostureDeviceTrust` turns a posture verdict into the `device-trusted` Ziti attribute, which is the Tier‑2 gate the dial policies require, so the journey's real question is not whether posture is recorded but **who is allowed to say what a device's posture is**. The answer was "anybody": `/agent/report` and `/agent/config` are registered outside the JWT middleware because an agent authenticates with its own enrollment credentials, and neither handler read them — `/agent/report` took the agent id from the JSON body and trusted it, `/agent/config` from a header or an `agent_id` query parameter. One unauthenticated request could grant a device Tier‑2 network access or strip a compliant laptop of it, and could write posture rows for any agent id at all; proven against the running service at `202 {"compliance_score":1}` with an id nobody had enrolled, and the row stored. Both shipped agents already send the credential (Android as `X-Auth-Token`, the Go agent as `Authorization: Bearer`) and `access-service.yaml` already documented the `401`, so no deployed agent is affected by the fix. The lookup now lives once in `internal/access/agent_auth.go` and the two handlers that already had a copy — both of which cite `/agent/report` in their comments as the pattern they follow — delegate to it. Eight assertions: the anonymous report, the guessed token, the never-enrolled id, the forged failure that would have *removed* a compliant device's trust, the device's own report landing in the trail and in the administrator's view, a report filed under another device's id, and the configuration read. J4's third step (reach a dark service) needs a controller, a router and a dark service to dial, and stays an operator drill; so do J8's backup, restore and upgrade steps (`tools/darkprobe`, `make dr-game-day`) to be filed under `docs/evidence/` (P8.4) |
