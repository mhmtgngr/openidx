...
func allMigrations() []*Migration {
	return []*Migration{
		...
		{
			Version:     191,
			Name:        "scim_list_ordering_index",
			Description: "Index users and groups on (org_id, created_at, id) so the SCIM list endpoints page in the order they now declare. SCIM cannot become a cursor the way the audit event list did: RFC 7644 3.4.2.4 pages a ListResponse by startIndex and REQUIRES totalResults, so the client picks the offset and the count has to be computed -- dropping either breaks the protocol rather than saving a scan. What could be fixed was the ordering, and it needed fixing for correctness before it needed an index: ORDER BY created_at alone is not a total order, and in this schema the ties are GUARANTEED rather than unlucky, because created_at defaults to NOW() and NOW() in Postgres is the TRANSACTION timestamp -- so every row one transaction writes carries the identical value, and a SCIM bulk create, a directory sync or a CSV import each lands a block of rows that tie exactly (measured: two hundred rows in one transaction, ONE distinct created_at). Tied rows with no second key may come back in any order and need not repeat it between executions, so under OFFSET paging a client walking the directory sees a user twice or not at all -- on a provisioning API that is an account that never reaches a downstream application, or one provisioned twice, and nothing logs it because every page was a valid answer. The list now orders by (created_at, id) from one place (internal/provisioning/scim_ordering.go, pinned by TestSCIMOrderingIsTotal) and this index answers that without a sort. It leads with org_id because every SCIM read is tenant-scoped first; a (created_at) index would be install-wide and would walk every tenant's rows to find yours, the defect v176 fixed for audit_events and v190 avoided. ASC rather than DESC because these lists page forward from the oldest row, which is the order the API has always presented. NOT built CONCURRENTLY: that cannot run inside a transaction block and applyMigration wraps every migration in one (which carries the bypass_rls GUC the seeds need), so on a large existing table this takes a write lock for the build -- stated rather than discovered. WHAT IT STILL DOES NOT FIX, because the protocol does not allow it: a row inserted ahead of the client's position between two pages shifts every later OFFSET by one and the row at the boundary is never returned -- measured as one user in two hundred silently missing after a single concurrent insert. That is a property of index-based paging; a cursor is what removes it, and SCIM has nowhere to put one. Down drops both indexes; the lists keep working, correctly (the tie-break is in the query, not the index) and with a sort.",
			UpSQL:       scimListIndexUp,
			DownSQL:     scimListIndexDown,
		},
		{
			Version:     192,
			Name:        "platform_outbox",
			Description: "The platform outbox: one table every service writes its domain events into, inside the transaction that produced them. An event describing a state change has to be atomic with it -- write the row then publish and a crash between the two loses the event with nothing to replay from; publish then write and the platform has announced something that did not happen. Neither window closes by retrying, because the failure is that nobody knows which side of it the process died on. Writing the event into the same transaction removes the window: one commit, or neither. THE ID IS NOT A CURSOR, and this is the trap the table is shaped around: a sequence hands out numbers at INSERT time while transactions commit in whatever order they finish, so a later id can become visible BEFORE an earlier one -- tx A takes 5 and is slow, tx B takes 6 and commits first, and a relay remembering 'published up to 6' never sees 5. Measured against a real PostgreSQL: with two concurrent transactions the lower id committed second in every run, and a cursor-based relay lost exactly that row while a state-based one delivered both. So the relay claims by published_at IS NULL with FOR UPDATE SKIP LOCKED, never by id -- the same shape the SCIM provisioning queue has used since v95 -- and the id exists to order a claimed batch and to name a row in a log line. The backlog index is PARTIAL (WHERE published_at IS NULL) because an outbox is append-mostly and read-once: within a day the published rows are the overwhelming majority and the relay never wants them, so a plain index would grow with the table while the query it answers would not. There is a second partial index on published_at for the retention sweep, so ageing out delivered rows does not walk the backlog index, and a UNIQUE (org_id, event_id) so a consumer can recognise a redelivery -- per tenant rather than globally, because a collision across tenants is a coincidence and must not make one tenant's write fail on another's. TENANCY IS BELTED AT CREATION, not in a later migration: org_id is NOT NULL with a real foreign key and the FORCE RLS policy lands with the table. The register's recurring finding is a nullable tenant column on a belted table, where a NULL-org row does not leak but DISAPPEARS -- here that would be an event no tenant-scoped reader can see and no operator can explain -- and a table belted later spends the interval unbelted, so every row written in it has to be trusted rather than checked. The relay is cross-tenant by design and reaches these rows under orgctx.WithBypassRLS, the same way the SCIM outbound worker and the SSF transmitter do: one relay for the install, claiming by state alone, with the tenant travelling on each claimed row into the subject it publishes to. Down drops the table; an outbox row is a message in flight and a rollback that kept them would keep them unreachable.",
			UpSQL:       outboxUp,
			DownSQL:     outboxDown,
		},
		{
		...
		{
			Version:     194,
			Name:        "webhook_delivery_claim_marker",
			Description: "Add webhook_deliveries.queued_at plus a partial index, so the webhook retry sweep claims what it enqueues instead of re-reading it. THE QUEUE IS REDIS AND THE RECORD IS POSTGRESQL, and nothing wrote down which deliveries were already queued. Publish INSERTs a row as pending and nudges Redis; processRetryBatch scans for pending rows and pushes them back, so a nudge lost to a Redis blip is still delivered -- that backstop is right. What it could not tell was a delivery nobody has queued from one the consumer simply has not reached yet, because a row stays pending for its whole life in the queue AND for the whole HTTP call. So every thirty seconds the entire undrained backlog was enqueued again: measured, four entries for one event after three ticks, and eighty for twenty deliveries when four sweeps ran at once, which is what a Redis outage does to the leader gate that was the only thing holding it to one. That is not at-least-once delivery -- at-least-once is a delivery that may repeat, this was a feedback loop whose output grew with how far behind the consumer already was, so it grew fastest exactly when the consumer was already struggling. WHY A SECOND COLUMN RATHER THAN REUSING next_retry_at: the two answer different questions, and the first attempt at this fix proved it by breaking. next_retry_at is WHEN A DELIVERY BECOMES DUE (the one, five and thirty minute backoff); queued_at is WHETHER SOMEONE ALREADY HAS IT. Folding the claim into next_retry_at made the sweep's own claim look, to the consumer, like a delivery that was not due yet, so the sweep enqueued the delivery and the consumer refused it and the customer's endpoint was called zero times. One column cannot hold both meanings because the sweep must be able to hand a row to a consumer it has just claimed. The claim also repairs the backoff, which was written down and defeated one line later: scheduleRetry stored next_retry_at and then pushed the id straight back onto the queue, where a consumer blocked on BRPop took it within milliseconds, so all three attempts were spent in under a second and the delivery was marked failed -- a retry policy meant to ride out a thirty-six minute outage rode out nothing, and a customer endpoint that restarted in ten seconds had already lost the event. The index is partial on status for the reason the outbox's backlog index is: a delivered webhook is never wanted by the sweep again and within a day the delivered rows are the overwhelming majority, so an index the size of the backlog answers the hot query at constant cost while the table grows. Existing rows get NULL, which reads as 'nobody has claimed this' -- the safe direction -- and the sweep keeps the original thirty second grace as the branch for them, so a delivery in flight across the upgrade is picked up once rather than lost. Additive and idempotent. Down drops both and the sweep reverts to delivering everything, and multiplying the queue while it does, rather than breaking.",
			UpSQL:       webhookDeliveryClaimUp,
			DownSQL:     webhookDeliveryClaimDown,
		},
		{
			Version:     195,
			Name:        "org_cells_tenant_directory",
			Description: "Create org_cells, the tenant directory: which cell serves which org, plus region, residency and status. THIS IS THE ROUTING DECISION, NOT THE BACKSTOP, and the two halves of plan task 4.1 are easy to conflate. cell.Guard answers 421 Misdirected Request to a token stamped with another cell -- that is what happens BEHIND a routing decision that was already made and was wrong. This table is the decision itself, the record the edge reads to send a request to the right cell in the first place. The directory makes the common case right and the 421 makes the uncommon case legible; neither substitutes for the other. IT ALSO CLOSES A GAP IN THE BACKSTOP, which is why it is not merely the nicer half: the issuer stamps the cell it MINTED a token in, which catches a token carried from one cell to another but not a LOGIN that reached the wrong cell. An eu-1 issuer serving a us-1 tenant mints cell=eu-1, every eu-1 guard compares eu-1 against eu-1, and the request is served from a database that does not hold that tenant -- the confident 404 the 421 exists to replace, arriving through the one door the guard cannot watch. With this table the issuer stamps the tenant's HOME cell and the guard in the wrong cell refuses on the first request rather than the hundredth. NO FOREIGN KEY TO organizations, which is the point rather than an omission: in a celled deployment the directory is GLOBAL and organizations is PER-CELL, so the row that says 'org X lives in us-1' is precisely a row about an org this cell's organizations table does not have, and a foreign key would make the directory unable to record the only fact it exists to record. The cost is stated rather than discovered -- nothing deletes a placement when an org is deleted, so the control plane owns that, and an orphaned placement names a tenant that is gone rather than corrupting one that is not. cell_id is CHECKed non-empty because cell.Misdirected treats an empty cell as 'this token predates the claim' and SERVES the request: a placement with an empty cell_id would read everywhere downstream as a tenant that is not placed at all -- placed nowhere and refused by nobody -- so the column refuses to hold it. The belt is org_id-scoped RLS, forced, applied at creation for v192's reason (a table that gets its policy later spends the interval unbelted); a tenant has no business reading where another tenant lives, and the readers that legitimately cross tenants do it under orgctx.WithBypassRLS the way the outbox relay, the SCIM outbound worker and the SSF transmitter already do. Down drops the table: a placement is a control-plane record rather than tenant data, and an install rolling this back is one that is not celled, where an empty directory and no directory are the same thing.",
			UpSQL:       orgCellsUp,
