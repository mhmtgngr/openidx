...
// TestMyApplicationsMatchesOverviewCount_ShowAllAppsWhenUnassigned pins the
// same catalogue/count agreement for the WithShowAllAppsWhenUnassigned
// fallback: GetMyApplications returns every enabled app for an unassigned
// user when the flag is on, and GetAccessOverview's AppsCount must apply the
// same fallback rather than reporting 0 while the catalogue is non-empty.
func TestMyApplicationsMatchesOverviewCount_ShowAllAppsWhenUnassigned(t *testing.T) {
	db, cleanup := setupPortalTestDB(t)
	if db == nil {
		return
	}
	...
	const (
		...
		appA = "33333333-0000-0000-0000-0000000000d1"
		appB = "33333333-0000-0000-0000-0000000000d2"
	)
	ctx := orgctx.With(context.Background(), orgctx.Org{ID: org})
	...
	seeds := []string{
		// No assignments at all for user — the fallback path.
		`INSERT INTO applications (id, name, enabled, org_id) VALUES
		   ('` + appA + `','Unassigned1',true,'` + org + `'),
		   ('` + appB + `','Unassigned2',true,'` + org + `')`,
	}
	for _, stmt := range seeds {
		if _, err := db.Pool.Exec(ctx, stmt); err != nil {
			t.Fatalf("seed: %v", err)
	...
	svc := NewService(db, zap.NewNop(), WithShowAllAppsWhenUnassigned(true))
	apps, err := svc.GetMyApplications(ctx, user)
	if err != nil {
		t.Fatalf("GetMyApplications: %v", err)
	}
	...
	if len(apps) != ov.AppsCount {
		t.Errorf("catalogue lists %d apps but the overview counts %d — the unassigned-fallback must apply to both", len(apps), ov.AppsCount)
	}
	if len(apps) != 2 {
		t.Errorf("got %d apps, want 2 (the unassigned-fallback returns every enabled app)", len(apps))
	}
}
