...
func (s *Service) ListBypassCodes(ctx context.Context, userID, status string, activeOnly bool, limit, offset int) ([]MFABypassCode, int, error) {
	...
	// Build query conditions with parameterized queries. Scope to the
	// caller's org via the target user's org_id (mfa_bypass_codes has
	// no org_id of its own); both the count and list queries join
	// users u, so this filter applies to both.
	conditions := fmt.Sprintf("u.org_id = $%d", 1)
	...
	// Count total (join users so the org filter in conditions applies)
	var total int
	s.db.Pool.QueryRow(ctx, "SELECT COUNT(*) FROM mfa_bypass_codes bc JOIN users u ON bc.user_id = u.id WHERE "+conditions, args...).Scan(&total)

	// Get codes
	//orgscope:ignore org filter (u.org_id) is applied via the interpolated conditions string, which the linter can't see
	query := fmt.Sprintf(`
