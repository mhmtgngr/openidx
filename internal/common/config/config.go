...
type Config struct {
	...
	// THE OUTBOX RELAY'S BROKER (global-scale plan tasks 3.1b and 3.2).
	//
	// NATSURL empty means the relay has no sink, and cmd/event-relay refuses to
	// start rather than running a drain loop that can never deliver. That is
	// deliberate: a relay with no broker looks healthy, claims rows, fails every
	// publish and leaves the backlog growing -- the failure is visible only in
	// the table, which is the last place anyone looks.
	NATSURL      string `mapstructure:"nats_url"`
	NATSUser     string `mapstructure:"nats_user"`
	NATSPassword string `mapstructure:"nats_password"`

	// The stream and the subject prefix have to agree with the broker's subject
	// PERMISSIONS, which the Helm chart writes from its own nats.subjectPrefix.
	// A relay permitted to publish under "openidx.>" and configured to publish
	// under something else is refused by the server on every event.
	EventStream        string `mapstructure:"event_stream"`
	EventSubjectPrefix string `mapstructure:"event_subject_prefix"`

	// OutboxKeepForHours ages out DELIVERED rows. Zero disables the sweep, which
	// is what SweepPublished already means by a zero KeepFor: the receipt is
	// worth keeping until someone decides how long, and a default that quietly
	// deletes evidence is worse than a table that grows visibly.
	OutboxKeepForHours int `mapstructure:"outbox_keep_for_hours"`
	...
	// PostureDeviceTrustGate drives the phone's Ziti tier from its reported
	// device posture: when an enrolled agent's latest posture report is compliant
	// (no critical/high posture failures) the backend grants the `device-trusted`
	// role attribute on its Ziti identity — the Tier-2 gate that unlocks
	// remote/PAM + admin surfaces beyond the Tier-1 minimum (self-service +
	// console) every enrolled identity already gets. On a failing report the
	// attribute is removed, dropping the device back to Tier-1.
	//
	//   "off"     (default) — do nothing; posture is recorded but never changes
	//                          Ziti attributes (today's behavior, unchanged).
	//   "observe" — log what WOULD change but do not modify attributes (safe to
	//               roll out; lets an admin watch before enforcing).
	...
	// AccessAPIRequireAuth forces the access-service data API (/api/v1/access/*)
	// onto the hard-blocking auth middleware even when APP_ENV=development. In dev
	// the group otherwise uses SoftAuth, which verifies a bearer when present but
	// does NOT reject anonymous callers — leaving the whole data surface (PAM
	// inventory, sessions, etc.) readable without a token. Set this (env
	// ACCESS_API_REQUIRE_AUTH=true) on any box reachable off-localhost so a
	// dev-mode install still refuses unauthenticated data reads. The Tier-0 public
	// door (POST /api/v1/access/enroll), agent enroll/report, APK download and
	// temp-access links are registered outside this group and stay reachable.
	// Default false preserves today's local-dev ergonomics. Production
	...
	// ZitiHTTPTimeoutSeconds bounds every Ziti controller management API call so a
	// slow/hung controller can't stall an enroll/reconcile. Default 8s (env
	// ZITI_HTTP_TIMEOUT_SECONDS). Calls also run through a circuit breaker.
	ZitiHTTPTimeoutSeconds int `mapstructure:"ziti_http_timeout_seconds"`
	...
	// Apache Guacamole integration (the "direct" PAM session broker — guacd
	...
	// target over the overlay (guacd → loopback → ziti-tunnel → edge-router).
	// When unset, ziti-reach entries fall back to the direct broker only if it can
	// see the loopback ports (normally it can't — keep them on separate brokers).
	...
	// GuacamoleHTTPTimeoutSeconds bounds every REST call to the Guacamole broker
	// so a slow/hung broker can't stall a user's PAM connect. Default 8s (env
	// GUACAMOLE_HTTP_TIMEOUT_SECONDS). Calls also run through a circuit breaker.
	GuacamoleHTTPTimeoutSeconds int `mapstructure:"guacamole_http_timeout_seconds"`
...
func setDefaults(v *viper.Viper, serviceName string) {
	...
	// Empty: no broker, and the relay refuses to start rather than pretending.
	v.SetDefault("nats_url", "")
	v.SetDefault("nats_user", "")
	v.SetDefault("nats_password", "")
	// These two match the Helm chart's own defaults (nats.subjectPrefix and the
	// stream the sink creates), so a chart-installed relay needs neither set.
	v.SetDefault("event_stream", "OPENIDX_EVENTS")
	v.SetDefault("event_subject_prefix", "openidx")
	// Off. See the field comment: a default that deletes receipts is worse than
	// a table that grows where somebody can see it.
...
func bindEnvVars(v *viper.Viper) {
	...
	envMappings := map[string]string{
		...
		// WebAuthn relying party. Without these bindings the WEBAUTHN_RP_ID /
		// WEBAUTHN_RP_ORIGINS env vars are ignored (viper's prefixed AutomaticEnv
