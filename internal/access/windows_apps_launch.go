...
func (s *Service) endPamSession(ctx context.Context, orgID, sessionID string) {
	...
	if _, err := s.db.Pool.Exec(ctx,
		...
		s.logger.Warn("endPamSession: update failed", zap.String("session_id", sessionID), zap.Error(err))
	}
}
...
func (s *Service) importDiscoveredApps(ctx context.Context, orgID, hostEntryID string, apps []discoveryApp, userID string) (created, updated int) {
	for _, a := range apps {
		...
		if err != nil {
			s.logger.Warn("importDiscoveredApps: upsert failed", zap.String("alias", alias), zap.Error(err))
			continue
		}
		if wasInsert {
			created++
