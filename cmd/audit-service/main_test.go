// Package main provides tests for the Audit Service entry point
package main
...
// TestAuditService_ConfigValidation tests configuration validation scenarios
func TestAuditService_ConfigValidation(t *testing.T) {
	...
	t.Run("Invalid port returns error", func(t *testing.T) {
		...
		_, err := config.Load("audit-service")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "port")
	})
}

// TestAuditService_ProductionConfigValidation tests production-specific validation
func TestAuditService_ProductionConfigValidation(t *testing.T) {
	tests := []struct {
		name        string
		cfg         *config.Config
		wantErr     bool
...
// TestAuditService_HealthEndpoints tests health check endpoint registration
func TestAuditService_HealthEndpoints(t *testing.T) {
...
// TestAuditService_RouteRegistration tests that audit service routes are registered
func TestAuditService_RouteRegistration(t *testing.T) {
	t.Run("Audit service registers expected routes", func(t *testing.T) {
		router := gin.New()
		log := zap.NewNop()
...
// TestAuditService_GracefulShutdown tests graceful shutdown behavior
...
// TestAuditService_MiddlewareConfiguration tests middleware setup
func TestAuditService_MiddlewareConfiguration(t *testing.T) {
...
// TestAuditService_ElasticsearchChecker tests the elasticsearch health checker
func TestAuditService_ElasticsearchChecker(t *testing.T) {
	t.Run("Elasticsearch checker implements HealthChecker", func(t *testing.T) {
		checker := &elasticsearchChecker{}
		assert.Equal(t, "elasticsearch", checker.Name())
		assert.False(t, checker.IsCritical()) // ES is optional for audit service
	})

	// Note: We can't test the actual Check method without a real ES client
	// because calling Ping on a nil client will panic. In production,
	// the checker is only created with a valid ES client.
}

// TestAuditService_AuditStreamConfiguration tests audit stream configuration
func TestAuditService_AuditStreamConfiguration(t *testing.T) {
	tests := []struct {
		name         string
		origins      string
		expectOrigin []string
		...
		{"Multiple origins", "https://example.com,https://app.example.com", []string{"https://example.com", "https://app.example.com"}},
		{"Empty (same-origin)", "", nil},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := &config.Config{
				AuditStreamAllowedOrigins: tt.origins,
			}

			origins := cfg.GetAuditStreamAllowedOrigins()
			assert.Equal(t, tt.expectOrigin, origins)
		})
	}
}

// TestAuditService_ProductionWarnings tests production warning generation
func TestAuditService_ProductionWarnings(t *testing.T) {
...
func BenchmarkAuditService_HealthCheck(b *testing.B) {
	router := gin.New()
	log := zap.NewNop()
	healthService := health.NewHealthService(log)
	healthService.SetVersion("test-version")
