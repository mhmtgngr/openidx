...
func main() {
	...
	metrics.NewTracedPool(db.Pool.Raw(), "audit-service").StartPoolStatsCollector(context.Background())
	...
	var costRedis *goredis.Client
	if redis != nil {
		costRedis = redis.RateLimitDB()
	}
	router.Use(middleware.TenantCostLimit(costRedis, middleware.CostConfig{
		Mode:   costMode,
