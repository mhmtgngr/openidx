// Package main provides tests for the Identity Service entry point
package main
...
// TestIdentityService_ConfigValidation tests configuration validation scenarios
func TestIdentityService_ConfigValidation(t *testing.T) {
	...
	t.Run("Invalid port returns error", func(t *testing.T) {
		...
		_, err := config.Load("identity-service")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "port")
	})
}

// TestIdentityService_ProductionConfigValidation tests production-specific validation
func TestIdentityService_ProductionConfigValidation(t *testing.T) {
	tests := []struct {
		name        string
		cfg         *config.Config
		wantErr     bool
...
// TestIdentityService_HealthEndpoints tests health check endpoint registration
func TestIdentityService_HealthEndpoints(t *testing.T) {
...
// TestIdentityService_RouteRegistration tests that identity service routes are registered
func TestIdentityService_RouteRegistration(t *testing.T) {
	t.Run("Identity service registers expected routes", func(t *testing.T) {
		router := gin.New()
		log := zap.NewNop()
...
// TestIdentityService_GracefulShutdown tests graceful shutdown behavior
func TestIdentityService_GracefulShutdown(t *testing.T) {
	t.Run("Shutdownable components are properly created", func(t *testing.T) {
		...
		cancelCtx := server.NewShutdownFunc("context", func(ctx context.Context) error {
		...
		assert.Equal(t, "context", cancelCtx.Name())
...
// TestIdentityService_ErrorHandling tests error handling scenarios
func TestIdentityService_ErrorHandling(t *testing.T) {
...
// TestIdentityService_MiddlewareConfiguration tests middleware setup
func TestIdentityService_MiddlewareConfiguration(t *testing.T) {
...
// TestIdentityService_AnomalyDetectorAdapter tests the anomaly detector adapter
func TestIdentityService_AnomalyDetectorAdapter(t *testing.T) {
	t.Run("AnomalyDetectorAdapter implements required methods", func(t *testing.T) {
		// The adapter should implement the required interface methods
		// This is a compile-time test
		adapter := &anomalyDetectorAdapter{}
		assert.NotNil(t, adapter)

		// Just verify the type exists and has the right structure
		// We can't call methods without a real risk service
		assert.NotNil(t, adapter)
	})
}

// TestIdentityService_SMSConfiguration tests SMS configuration
func TestIdentityService_SMSConfiguration(t *testing.T) {
	tests := []struct {
		name     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.enabled {
				assert.Equal(t, tt.provider, tt.cfg.Provider)
			}
		})
	}
}
...
func TestIdentityService_WebAuthnConfiguration(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Production WebAuthn config",
			cfg: config.WebAuthnConfig{
				RPID:      "example.com",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.rpID, tt.cfg.RPID)
			assert.Equal(t, tt.origins, tt.cfg.RPOrigins)
			assert.Greater(t, tt.cfg.Timeout, 0)
		})
	}
}

// TestIdentityService_ProductionWarnings tests production warning generation
func TestIdentityService_ProductionWarnings(t *testing.T) {
...
func BenchmarkIdentityService_HealthCheck(b *testing.B) {
	router := gin.New()
	log := zap.NewNop()
	healthService := health.NewHealthService(log)
	healthService.SetVersion("test-version")
