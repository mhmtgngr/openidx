...
func main() {
	...
	// Body cap (settings and branding writes carry inline images); oversize is 413 before any handler runs (task 0.3).
	router.Use(middleware.MaxBodySize(5 << 20))
	...
	// On the v1 group, not the engine: gin copies an engine's middleware into a
	// group when the group is CREATED, so a router.Use() here -- after v1
	// exists -- would never run for any route under it.
	v1.Use(middleware.TenantCostLimit(redis.RateLimitDB(), middleware.CostConfig{
		Mode:   costMode,
