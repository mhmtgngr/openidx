...
- **identity-service runs as one plane or the other (`SERVICE_PROFILE`).** The
  ...
  **both** profiles, because a route that quietly vanished from a running
  ...
  The classification is published as data too —
  `helm/openidx/files/identity-planes.json`, generated by `go run
  ./tools/identityplanes` and held in step with the table by a test — so the
  edge routing rules can be generated from it instead of retyped beside it.
  That routing is still to come, and it is not a prefix: `POST
  /users/forgot-password` is ISSUE while `GET /users` is ADMIN, so one
  `pathType: Prefix` rule cannot separate them.
