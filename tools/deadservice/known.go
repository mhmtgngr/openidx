...
var knownDead = map[string]string{
	...
	"internal/risk.AlertManager": "security alert generation, deduplication, severity routing and delivery to a security-team mailing list, over its own AlertConfig. Nothing constructs it. risk.Service and risk.Scorer are the live risk path and neither raises an alert through this. A FEATURE THAT LOOKS SHIPPED: today a high-risk login scores and is recorded, and no one is told. Wire it into the scorer's high-severity branch, or delete it and stop implying alerts exist.",
